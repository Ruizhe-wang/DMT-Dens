...
class DMTBaseDataModule(pl.LightningDataModule):
    ...
    def load_data(self, data_path, sample_data_size=60000):
        ...
        adata = sc.read("/root/hanning/mulleveldr/m1data/celegan.h5ad")
        label_celltype = pd.read_csv('/root/hanning/mulleveldr/m1data/celegan_celltype_2.tsv', sep='\t', header=None)
        label_embryo_time = pd.read_csv('/root/hanning/mulleveldr/m1data/celegan_embryo_time.tsv', sep='\t', header=None)
        ...
        # sc.pp.filter_genes(adata, min_counts=100)      # 至少 1 个 counts
        # sc.pp.filter_genes(adata, min_cells=adata.X.shape[0]*0.01)       # 至少在 3 个细胞中出现过
        ...
        # sc.pp.highly_variable_genes(adata, n_top_genes=150, flavor="cell_ranger")
        
        adata.obs['batch'] = adata.obs['embryo_time']
        ...
        # import pdb; pdb.set_trace()
        annotation = list(adata.obs['cell_type'])
