...
class DMTBaseDataModule(pl.LightningDataModule):
    ...
    def load_data(self, data_path, sample_data_size=60000):
        ...
        adata = sc.read_h5ad("/root/hanning/mulleveldr/m1data/jky_liu.h5ad")
        ...
        X_top500 = np.asarray(X_dense[:, mask_gene])

        str_label = adata_tmp.obs['sample_id'].to_list()
        labels = np.array(str_label)

        label_dict = {
            'W10251420483': 'WT-xin-1', 
            'W10251420484': 'WT-xin-2', 
            'W10251420485': 'WT-xin-3', 
            'W10251420486': 'kr-xin-1',
            'W10251420487': 'kr-xin-2',
            'W10251420488': 'kr-xin-3',
            'W10251420489': 'WT-fei-1',
            'W10251420490': 'WT-fei-2',
            'W10251420491': 'WT-fei-3',
            'W10251420492': 'kr-fei-1',
            'W10251420493': 'kr-fei-2',
            'W10251420494': 'kr-fei-3',
        }

        # 转换为映射后的标签
        labels_full = np.array([label_dict.get(str(l), str(l)) for l in labels])

        # 只保留 WT-xin 和 kr-xin
        mask = np.array([("WT-xin" in l) or ("kr-xin" in l) for l in labels_full])
        adata_tmp = adata_tmp[mask, :].copy()
        X_top500 = X_top500[mask, :]

        # WT -> 1, KR -> 0
        binary_labels = np.array([1 if "WT-xin" in l else 0 for l in labels_full[mask]])
        
        sc.pp.neighbors(adata_tmp, n_neighbors=10, n_pcs=50)
        sc.tl.leiden(adata_tmp, resolution=0.5)
        
        adata_tmp.obs['batch'] = adata_tmp.obs['leiden']
        adata_tmp.obs['final_annotation'] = adata_tmp.obs['leiden']
        adata_tmp.obs['cell_type'] = adata_tmp.obs['leiden']
        
        # import pdb; pdb.set_trace()
        return adata_tmp, X_top500, binary_labels
