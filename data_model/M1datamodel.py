...
class DMTBaseDataModule(pl.LightningDataModule):
    ...
    def load_data(self, data_path, sample_data_size=60000):
        ...
        adata = sc.read("/root/Immune_ALL_human.h5ad")
        self.adata = adata
    ...
    def cal_near_index(self, k=10, device="cuda", uselabel=False, pca_dim=100):
        filename = "save_near_index/data_name{}K{}uselabel{}pcadim{}.pkl".format(
            self.__class__.__name__+str(self.data.shape), k, uselabel, pca_dim
        )
    ...
    def setup(self, stage: str):
        ...
        adata, data, label = self.load_data(self.data_path, sample_data_size=self.sample_data_size)
        
        augmenter = Augmenter([
            {"name": "TraceMixup", "alpha_max": 1.0, "p": 1.0, "n_hop": 2},  # p=1 表示每次都做
            # {"name": "GaussianNoise", "std": 0.05, "p": 0.5}        # p=0.5 表示 50% 概率做
        ])
        
        
        # data = np.log1p(data)  # 对数据进行log1p变换
        # mean = data.mean()
        # std = data.std()
        # data = (data - mean) / std
        
        # pca_projector = PCA(n_components=50)
        # data = pca_projector.fit_transform(data)
        self.adata = adata
        self.data = data
        self.label = label
        ...
        self.dataset = DataSetBase(
            data=self.data,
            label=self.label,
            neighbors_index=neighbors_index,  # 这里可以根据需要设置邻居索引
            transform=augmenter,
        )
