...
def plot_radial_tree_major_class(root, path_list=None):
    # 1) 收集所有 major_class
...
def plot_radial_tree_anytree(
    ...
    *,
    ...
    path_list=None,                # list of list of nodes, to highlight certain paths
    ):
    ...
    for path in path_list:
        ...
        path_linage_y_list = []
        for node in path:
            n_real = find_by_attr(root, name="name", value=node)
            loc = xy[n_real]
            path_linage_x_list.append(loc[0])
            path_linage_y_list.append(loc[1])
        linage_x_list.append(path_linage_x_list)
        linage_y_list.append(path_linage_y_list)
