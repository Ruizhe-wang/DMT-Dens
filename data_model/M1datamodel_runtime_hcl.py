...
class DMTBaseDataModule(pl.LightningDataModule):
    ...
    def load_data(self, data_path, train=True):
        ...
        obs = sadata.obs.iloc[mask_label].copy()
        var = sadata.var.copy()
        if var.index.empty:
        ...
        annotation_key = None
        for candidate in ["celltype", "cell_type", "final_annotation", "tissue", "louvain"]:
            if candidate in obs:
        ...
        adata.obs['final_annotation'] = annotation_values
        if 'cell_type' not in adata.obs:
            adata.obs['cell_type'] = annotation_values
        if 'celltype' not in adata.obs:
            adata.obs['celltype'] = annotation_values
