...
class DMTBaseDataModule(pl.LightningDataModule):
    ...
    def load_data(self, data_path):
        data_file = os.path.join(data_path, "dmthi/aqc_all_data_3000.npy")
        label_file = os.path.join(data_path, "dmthi/aqc_all_label.npy")
        if not os.path.exists(data_file):
            raise FileNotFoundError(f"AQC data file not found: {data_file}")
        if not os.path.exists(label_file):
            raise FileNotFoundError(f"AQC label file not found: {label_file}")
