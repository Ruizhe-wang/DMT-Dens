...
densne.run_densne(data, no_dims=2, perplexity=50, theta=0.5, rand_seed=-1,
                  ...
                  max_iter=1000, dens_frac=0.3, dens_lambda=0.1,
                  final_dens=True, early_exaggeration=12.0)
```
...
dens_frac = 0.3; dens_lambda = 0.1; final_dens = true; early_exaggeration = 12.0;

[embedding, ro, re] = run_densne(data, no_dims, initial_dims, perplexity, theta, alg, max_iter, ...
                                 dens_frac, dens_lambda, final_dens, [], early_exaggeration);
...
* `early_exaggeration`: Positive multiplicative factor applied to input similarities during the
initial optimization iterations (float), default 12.0
...
python densne.py -h [--help HELP] -i [--input INPUT] -o [--outname OUTNAME]
                 ...
                 -t [--theta THETA] -e [--early_exaggeration EARLY_EXAGGERATION]
                 -r [--randseed RANDSEED]
