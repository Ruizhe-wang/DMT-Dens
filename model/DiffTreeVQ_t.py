...
from model.model_trans import XFormerDMTEncoderVec
...
class DMTEVT_model(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enc = XFormerDMTEncoderVec(
            num_layers=self.hparams.T_num_layers,
            # num_layers=3,
            num_attention_heads=self.hparams.T_num_attention_heads,
            num_input_dim_pad=num_input_dim_pad,
            # num_attention_heads=6,
            # hidden_size=self.hparams.T_hidden_size,
            hidden_size=240,
            # intermediate_size=self.hparams.T_intermediate_size,
            intermediate_size=300,
            seq_len=seq_len,                 # 用能整除的 L
            # num_input_dim=self.hparams.num_input_dim,  # 向量长度
            num_input_dim=num_input_dim,  # 向量长度
            # attention_probs_dropout_prob=self.hparams.T_attention_probs_dropout_prob,
            attention_probs_dropout_prob=0.1,
            output_dim=t_output_dim,
            # attention_probs_dropout_prob=self.hparams.T_attention_probs_dropout_prob,
            # output_dim=self.hparams.t_output_dim,      # **关键：让编码器输出等于 t_output_dim**
        )
    ...
    def LossManifold_Global(
        ...
        with torch.no_grad():
            # select to 10 largest loss in each row
            topk_values, _ = torch.topk(loss, k=100, dim=1)
