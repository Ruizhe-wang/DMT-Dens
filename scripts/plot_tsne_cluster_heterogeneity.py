"""
Generate four figures that expose hidden heterogeneity inside visually smooth t-SNE clusters.

Same analysis as the UMAP version, but uses t-SNE for the 2D embedding.

Outputs:
  1. t-SNE colored by high-dimensional local scale
  2. t-SNE colored by local distortion ratio (low-dim / high-dim)
  3. Within-cluster pairwise distance matrix comparison
  4. Ordered sample strip along the cluster direction

Examples:
  python scripts/plot_tsne_cluster_heterogeneity.py --labels 1 7 --target-label 1
  python scripts/plot_tsne_cluster_heterogeneity.py --target-label 1
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize hidden heterogeneity inside visually smooth t-SNE clusters."
    )
    parser.add_argument(
        "--input-npz",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Label for within-cluster views (figures 3 and 4). Defaults to the largest class.",
    )
    parser.add_argument(
        "--max-samples",
        type=int,
        default=4000,
        help="Maximum total samples used to fit t-SNE and compute local metrics.",
    )
    parser.add_argument(
        "--cluster-max-samples",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="results/tsne_cluster_heterogeneity",
...
# ---------------------------------------------------------------------------
# Data loading (shared logic with UMAP version)
# ---------------------------------------------------------------------------
...
def plot_distance_matrix_comparison(
    ...
    panels = [
        ...
        (ld_norm, "t-SNE Pairwise Distance", "magma", 0.0, vmax),
    ...
    fig.suptitle(
        f"Within-cluster distance comparison for label {target_label} (t-SNE)",
        fontsize=13,
    )
    fig.tight_layout()
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
...
def plot_sample_strip(
    ...
    ax_scatter.set_title(f"Ordered traversal across t-SNE cluster {target_label}", fontsize=13)
...
def main() -> None:
    ...
    print(f"Running t-SNE on {len(data)} samples (perplexity={args.perplexity}, "
          f"lr={args.learning_rate}, n_iter={args.n_iter}) ...")

    tsne = TSNE(
        n_components=2,
        perplexity=min(args.perplexity, len(data) - 1),
    ...
    embedding = np.asarray(tsne.fit(data))
    ...
    fig1_path = output_dir / "01_tsne_local_scale.png"
    fig2_path = output_dir / "02_tsne_local_distortion.png"
    ...
    plot_tsne_metric(
        ...
        labels=labels,
        metric=local_scale,
        title="t-SNE colored by high-dimensional local scale",
    ...
    plot_tsne_metric(
        ...
        labels=labels,
        metric=distortion,
        title="t-SNE colored by local distortion ratio",
    ...
    print("=" * 72)
    print("t-SNE cluster heterogeneity figures saved:")
